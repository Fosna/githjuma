  ...
  if (!isset($_POST['comment-submit'])) {
    ...
    if (empty($postname) || empty($comment)){
      ...
      if (!isset($_SESSION['id'])) {
        ...
        $sql1 = "INSERT INTO hjuma_comments (grouppost, post, commenter, comment, date_time, avatarname, avatar) VALUES (?, ?, ?, ?, ?, ?, '$image');";
        $stmt = mysqli_stmt_init($conn);
        if (!mysqli_stmt_prepare($stmt, $sql1)) {
          echo "SQL error";
        }else {
          mysqli_stmt_bind_param($stmt,"ssssss", $groupname, $postname, $commenter, $comment, $date, $image_name);
